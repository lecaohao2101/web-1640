...
const [rows2] = await connection.query(
  "SELECT * FROM student INNER JOIN departmentManager ON departmentManager.department_id = student.student_department_id WHERE student.student_id= ?",
  [student_id]
);
if (rows2.length > 0) {
  ...
  try {
    ...
    res.status(500).json({ err: "Send mail error" });
    return; 
  }
}
